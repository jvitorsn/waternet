...
This chapter presents the experimental results obtained from the four neural network architectures described in Section~\ref{subsec:models}, evaluated first on the held-out synthetic test partition under Subsection~\ref{subsec:synth_results}, and then on a real-world dataset acquired from a human piloted UAV in a coastal site in Subsection~\ref{subsec:real_results}. Subsection~\ref{subsec:model_comparison} provides a comparative analysis of the architectures in terms of size, complexity, and practical deployment considerations. The synthetic and real datasets, trained CNN models, Blender models and used algorithms are available at the repository \url{https://github.com/jvitorsn/waternet}.
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{Images/WaterNet_histogram.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{Images/WaterNet-FusLT_histogram.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{Images/ResNet50_histogram.png}
...
\paragraph{ResNet50-Fusion (Fig.~\ref{fig:resnet50fus_hist}).} The fusion model substantially narrows the error distribution ($\mu = 0.8$~cm, $\sigma = 44.3$~cm), reducing the standard deviation by more than half compared to the baseline. The distribution is approximately symmetric and more sharply peaked, indicating that the majority of the inferences fall within $\pm50$~cm of the true altitude. The tail extent is also reduced, with errors rarely exceeding $\pm200$~cm.

\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{Images/ResNet50-Fus_histogram.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{Images/plots/lidar_plot.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{Images/plots/waternet_real_plot.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{Images/plots/resnet50_real_plot.png}
...
The LiDAR achieves the lowest MAE overall (1.697~m) but the highest RMSE (4.364~m) among all methods, a diagnostic pattern indicating the presence of extreme outliers. This large gap between MAE and RMSE is characteristic of heavy-tailed error distributions: the LiDAR produces accurate readings for a subset of frames (reflected in its low median absolute error of 0.050~m) but returns severely erroneous measurements in others, inflating the squared-error penalty. In practical terms, a sensor with the lowest MAE but the highest RMSE is unreliable for safety-critical altitude control, where worst-case performance matters more than average accuracy.
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=14cm]{Images/benchmark_bias.png}
